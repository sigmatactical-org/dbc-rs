...
#[test]
fn test_limit_error_messages_report_configured_limits() {
    // The *_TOO_MANY texts are generated by build.rs from the DBC_MAX_* env
    // vars; they must always name the limit that is actually compiled in.
    use dbc_rs::{Error, MAX_NODES, MAX_SIGNALS_PER_MESSAGE};

    assert!(
        Error::MESSAGE_TOO_MANY_SIGNALS.contains(&format!(
            "maximum allowed is {MAX_SIGNALS_PER_MESSAGE} per message"
        )),
        "stale signal limit in: {}",
        Error::MESSAGE_TOO_MANY_SIGNALS
    );
    assert!(
        Error::NODES_TOO_MANY.contains(&format!("maximum allowed is {MAX_NODES}")),
        "stale node limit in: {}",
        Error::NODES_TOO_MANY
    );
}

#[test]
fn test_too_many_signals_reports_message_error_with_limit() {
    // One more signal than the compiled-in per-message capacity.
    use dbc_rs::MAX_SIGNALS_PER_MESSAGE;

    let mut dbc_str = String::from("VERSION \"1.0\"\n\nBU_: A B\n\nBO_ 1 M : 8 A\n");
    for i in 0..=MAX_SIGNALS_PER_MESSAGE {
        dbc_str.push_str(&format!(" SG_ S{i} : {i}|1@1+ (1,0) [0|1] \"\" B\n"));
    }

    let err = Dbc::parse(&dbc_str).expect_err("must exceed the signal capacity");
