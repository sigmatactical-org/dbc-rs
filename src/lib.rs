...
// - DBC_MAX_MESSAGES (default: 16, must be power of 2 for heapless)
// - DBC_MAX_SIGNALS_PER_MESSAGE (default: 8)
// - DBC_MAX_NODES (default: 8)
// - DBC_MAX_VALUE_DESCRIPTIONS (default: 16)
// - DBC_MAX_NAME_SIZE (default: 32, per DBC specification)
// - DBC_MAX_EXTENDED_MULTIPLEXING (default: 16)
// - DBC_MAX_ATTRIBUTE_DEFINITIONS / _VALUES / _ENUM_VALUES (32 / 64 / 16)
//
// Defaults are sized for embedded safety-bus DBCs: in `heapless` mode these
// capacities are inline storage, so size_of::<Dbc>() scales with their
// product — the old 8192-message default produced multi-gigabyte debug
// objects (issue #2). Large industrial DBCs: use `alloc`/`std`, or raise
// the env vars at build time.
